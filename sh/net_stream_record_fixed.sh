...
for i in $(seq 0 $((N-1))); do
  ...
  ffmpeg -hide_banner -loglevel error -fflags +genpts -flags low_delay \
  -f v4l2 -input_format mjpeg -framerate 15 -video_size 1280x720 -i /dev/video2 \
  -filter_complex "[0:v]split=2[vf][vtmp];[vtmp]fps=10,scale=640:480,format=nv12[vn]" \
  -map "[vf]" -vsync vfr -c:v mjpeg -q:v 5 -t 120 -f avi /dev/shm/test_cam2.avi \
  -map "[vn]" -vsync cfr -r 10 -c:v libx264 -preset ultrafast -tune zerolatency \
      -x264-params keyint=20:scenecut=0 -g 20 -bf 0 -threads 2 \
  -f mpegts -muxdelay 0 -muxpreload 0 srt://0.0.0.0:9000?mode=listener&latency=80
  sleep 0.2
done
