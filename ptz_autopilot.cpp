...
struct SpeedPoint { int profile_idx=-1; int step=-1; double zoom_ratio=0.0,focal_px=0.0,kp=0,ki=0,kd=0,deadzone=0,ptz_curve=1.0; int max_pan=0,max_tilt=0,max_accel=0,min_pan=0,min_tilt=0,ptz_lead_ms=0,j_pulse_ms=70,nudge_pan=0,nudge_tilt=0; std::string manual_mode="tap_hold"; };
...
struct MovementResult { int profile_idx=-1; int zoom_sample_idx=-1; double zoom_ratio=0.0; int base_pan=0; int base_tilt=0; int final_pan=0; int final_tilt=0; int effective_max_pan=0; int effective_max_tilt=0; int effective_max_accel=0; int j_pulse_ms=70; int nudge_pan=0,nudge_tilt=0; std::string manual_mode="tap_hold"; std::string source; std::string speed_profile_source="fallback"; int left_profile_idx=-1; int right_profile_idx=-1; double interpolation_t=0.0; };
...
static std::string speed_point_to_json(const SpeedPoint& p){ std::ostringstream os; os<<"{\"profile_idx\":"<<p.profile_idx<<",\"step\":"<<p.step<<",\"zoom_ratio\":"<<p.zoom_ratio<<",\"focal_px\":"<<p.focal_px<<",\"kp\":"<<p.kp<<",\"ki\":"<<p.ki<<",\"kd\":"<<p.kd<<",\"deadzone\":"<<p.deadzone<<",\"ptz_curve\":"<<p.ptz_curve<<",\"ptz_lead_ms\":"<<p.ptz_lead_ms<<",\"max_pan\":"<<p.max_pan<<",\"max_tilt\":"<<p.max_tilt<<",\"max_accel\":"<<p.max_accel<<",\"min_pan\":"<<p.min_pan<<",\"min_tilt\":"<<p.min_tilt<<",\"j_pulse_ms\":"<<p.j_pulse_ms<<",\"nudge_pan\":"<<p.nudge_pan<<",\"nudge_tilt\":"<<p.nudge_tilt<<",\"manual_mode\":\""<<p.manual_mode<<"\",\"source\":\"user\"}"; return os.str(); }
static std::vector<SpeedPoint> load_speed_profile_points(){ std::ifstream f(g_speedProfileFile); if(!f) return {}; std::string body((std::istreambuf_iterator<char>(f)), std::istreambuf_iterator<char>()); std::vector<SpeedPoint> pts; size_t pos=0; while(true){ auto keyPos=body.find("\"profile_idx\"",pos); if(keyPos==std::string::npos) keyPos=body.find("\"step\"",pos); if(keyPos==std::string::npos) break; auto objStart=body.rfind('{',keyPos); auto objEnd=body.find('}',keyPos); if(objStart==std::string::npos||objEnd==std::string::npos) break; std::string obj=body.substr(objStart,objEnd-objStart+1); SpeedPoint p; json_int(obj,"profile_idx",p.profile_idx); if(!json_int(obj,"step",p.step)) p.step=p.profile_idx; if(p.profile_idx<0) p.profile_idx=p.step; json_num(obj,"zoom_ratio",p.zoom_ratio); json_num(obj,"focal_px",p.focal_px); json_num(obj,"kp",p.kp); json_num(obj,"ki",p.ki); json_num(obj,"kd",p.kd); json_num(obj,"deadzone",p.deadzone); json_num(obj,"ptz_curve",p.ptz_curve); json_int(obj,"ptz_lead_ms",p.ptz_lead_ms); json_int(obj,"max_pan",p.max_pan); json_int(obj,"max_tilt",p.max_tilt); json_int(obj,"max_accel",p.max_accel); json_int(obj,"min_pan",p.min_pan); json_int(obj,"min_tilt",p.min_tilt); json_int(obj,"j_pulse_ms",p.j_pulse_ms); json_int(obj,"nudge_pan",p.nudge_pan); json_int(obj,"nudge_tilt",p.nudge_tilt); p.manual_mode=json_str(obj,"manual_mode"); if(p.manual_mode!="pulse"&&p.manual_mode!="hold"&&p.manual_mode!="tap_hold") p.manual_mode="tap_hold"; p.j_pulse_ms=std::max(1,std::min(120,p.j_pulse_ms)); pts.push_back(p); pos=objEnd+1; } return pts; }
static void clamp_speed_point(SpeedPoint& p){ p.kp=std::max(0.0,std::min(100.0,p.kp)); p.ki=std::max(0.0,std::min(20.0,p.ki)); p.kd=std::max(0.0,std::min(50.0,p.kd)); p.deadzone=std::max(0.0,std::min(0.5,p.deadzone)); p.ptz_curve=std::max(0.35,std::min(2.5,p.ptz_curve)); p.ptz_lead_ms=std::max(0,std::min(500,p.ptz_lead_ms)); p.max_pan=std::max(1,std::min(100,p.max_pan)); p.max_tilt=std::max(1,std::min(100,p.max_tilt)); p.max_accel=std::max(1,std::min(100,p.max_accel)); p.min_pan=std::max(0,std::min(p.max_pan,p.min_pan)); p.min_tilt=std::max(0,std::min(p.max_tilt,p.min_tilt)); p.j_pulse_ms=std::max(1,std::min(120,p.j_pulse_ms)); p.nudge_pan=std::max(0,std::min(100,p.nudge_pan)); p.nudge_tilt=std::max(0,std::min(100,p.nudge_tilt)); if(p.manual_mode!="pulse"&&p.manual_mode!="hold"&&p.manual_mode!="tap_hold") p.manual_mode="tap_hold"; }

static SpeedPointResolveResult resolve_speed_point_for_sample(int profile_idx,double zoom_ratio,double focal_px){
  auto pts=load_speed_profile_points(); std::sort(pts.begin(),pts.end(),[](const SpeedPoint&a,const SpeedPoint&b){return a.profile_idx<b.profile_idx;}); SpeedPointResolveResult r;
  for(const auto& p:pts){ if(p.profile_idx==profile_idx){ r.point=p; r.source="user"; return r; } }
  int li=-1,ri=-1; for(size_t i=0;i<pts.size();++i){ if(pts[i].profile_idx<profile_idx) li=(int)i; if(pts[i].profile_idx>profile_idx){ ri=(int)i; break; } }
  if(li>=0 && ri>=0){ const auto& L=pts[(size_t)li]; const auto& R=pts[(size_t)ri]; double lx=L.focal_px, rx=R.focal_px, x=focal_px; if(!(x>0&&lx>0&&rx>0&&rx>lx)){ lx=L.zoom_ratio; rx=R.zoom_ratio; x=zoom_ratio; } if(!(rx>lx)){ lx=L.profile_idx; rx=R.profile_idx; x=profile_idx; } double t=(rx>lx)?((x-lx)/(rx-lx)):0.0; t=std::max(0.0,std::min(1.0,t)); r.point=L; r.point.profile_idx=profile_idx; r.point.step=profile_idx; r.point.zoom_ratio=zoom_ratio; r.point.focal_px=focal_px; r.point.kp=L.kp+(R.kp-L.kp)*t; r.point.kd=L.kd+(R.kd-L.kd)*t; r.point.deadzone=L.deadzone+(R.deadzone-L.deadzone)*t; r.point.ptz_curve=L.ptz_curve+(R.ptz_curve-L.ptz_curve)*t; r.point.ptz_lead_ms=(int)std::lround(L.ptz_lead_ms+(R.ptz_lead_ms-L.ptz_lead_ms)*t); r.point.max_pan=(int)std::lround(L.max_pan+(R.max_pan-L.max_pan)*t); r.point.max_tilt=(int)std::lround(L.max_tilt+(R.max_tilt-L.max_tilt)*t); r.point.max_accel=(int)std::lround(L.max_accel+(R.max_accel-L.max_accel)*t); r.point.min_pan=(int)std::lround(L.min_pan+(R.min_pan-L.min_pan)*t); r.point.min_tilt=(int)std::lround(L.min_tilt+(R.min_tilt-L.min_tilt)*t); r.point.j_pulse_ms=(int)std::lround(L.j_pulse_ms+(R.j_pulse_ms-L.j_pulse_ms)*t); r.point.nudge_pan=(int)std::lround(L.nudge_pan+(R.nudge_pan-L.nudge_pan)*t); r.point.nudge_tilt=(int)std::lround(L.nudge_tilt+(R.nudge_tilt-L.nudge_tilt)*t); r.point.manual_mode=(t<0.5?L.manual_mode:R.manual_mode); clamp_speed_point(r.point); r.source="interpolated"; r.left_profile_idx=L.profile_idx; r.right_profile_idx=R.profile_idx; r.t=t; return r;
  }
  if(li>=0){ r.point=pts[(size_t)li]; r.source="clamped_left"; r.left_profile_idx=r.point.profile_idx; return r; }
  if(ri>=0){ r.point=pts[(size_t)ri]; r.source="clamped_right"; r.right_profile_idx=r.point.profile_idx; return r; }
  r.point.profile_idx=profile_idx; r.point.step=profile_idx; r.point.zoom_ratio=zoom_ratio; r.point.focal_px=focal_px; r.point.kp=cfg.kp; r.point.ki=cfg.ki; r.point.kd=cfg.kd; r.point.deadzone=cfg.deadzone; r.point.ptz_curve=1.0; r.point.ptz_lead_ms=0; r.point.max_pan=cfg.max_pan; r.point.max_tilt=cfg.max_tilt; r.point.max_accel=cfg.max_accel; r.point.min_pan=cfg.min_pan; r.point.min_tilt=cfg.min_tilt; r.point.j_pulse_ms=70; r.point.nudge_pan=cfg.max_pan; r.point.nudge_tilt=cfg.max_tilt; r.point.manual_mode="tap_hold"; r.source="fallback"; return r;
}
static bool save_speed_profile_points(const std::vector<SpeedPoint>& pts){ std::ofstream f(g_speedProfileFile); if(!f) return false; f<<"{\n  \"method\": \"ptz_speed_by_zoom_sample\",\n  \"version\": 2,\n  \"source_zoom_profile\": \"zoom_apriltag_master_profile.json\",\n  \"points\": [\n"; for(size_t i=0;i<pts.size();++i){ f<<"    "<<speed_point_to_json(pts[i]); if(i+1<pts.size()) f<<","; f<<"\n"; } f<<"  ]\n}\n"; return true; }

static std::string speed_profile_json(const std::vector<SpeedPoint>& pts){ std::ostringstream os; os<<"{\"ok\":true,\"points\":["; for(size_t i=0;i<pts.size();++i){ if(i) os<<","; os<<speed_point_to_json(pts[i]); } os<<"]}"; return os.str(); }
...
static bool fetch_zoom_state(ZoomState& zs){ std::string b; if(!http_get(cfg.tracker_host,cfg.tracker_port,"/api/zoom/state",b)){zs.error="zoom_state_unreachable"; return false;} zs.ok=true; json_bool(b,"profile_loaded",zs.profile_loaded); json_int(b,"zoom_sample_idx",zs.zoom_sample_idx); json_num(b,"zoom_ratio",zs.zoom_ratio); json_num(b,"focal_px",zs.focal_px); json_num(b,"focal_min_px",zs.focal_min_px); json_num(b,"focal_max_px",zs.focal_max_px); zs.source=json_str(b,"zoom_source"); if(!zs.profile_loaded) zs.error="zoom_profile_missing"; return true; }
...
static std::mutex g_manualPulseMtx;
static int clampi(int v,int lo,int hi){ return std::max(lo,std::min(hi,v)); }
static bool send_bridge_line(const std::string& line){
  static WsClient ws;
...
static double signed_curve(double e,double deadzone,double curve){ double ae=std::abs(e); deadzone=std::max(0.0,std::min(0.95,deadzone)); curve=std::max(0.35,std::min(2.5,curve)); if(ae<=deadzone) return 0.0; double n=(ae-deadzone)/(1.0-deadzone); double c=std::pow(std::max(0.0,std::min(1.0,n)),curve); return (e<0.0)?-c:c; }
static double clampd(double v,double lo,double hi){ return std::max(lo,std::min(hi,v)); }

static void autopilot_loop(){WsClient ws; auto last=std::chrono::steady_clock::now(); double ix=0,iy=0,pex=0,pey=0; int prevp=0,prevt=0; bool sent_stop=false; double smoothScale=1.0; while(g.run){auto start=std::chrono::steady_clock::now(); TrackerState ts; std::string body; bool ok=http_get(cfg.tracker_host,cfg.tracker_port,"/api/tracker/state",body)&&parse_tracker(body,ts); int pan=0,tilt=0,basePan=0,baseTilt=0; double ex=0,ey=0,predEx=0,predEy=0; ZoomState zs; fetch_zoom_state(zs); auto sr=resolve_active_speed_point_for_zoom(zs); SpeedPoint sp=sr.point; const bool hasSpeedProfile=(sr.source!="fallback"); const double rawScale=hasSpeedProfile?1.0:compute_zoom_speed_scale(zs); const double speedScale=(cfg.zoom_scale_smoothing<=0.0)?rawScale:(smoothScale=smoothScale*(1.0-cfg.zoom_scale_smoothing)+rawScale*cfg.zoom_scale_smoothing); bool active=g.enabled.load(); bool needSend=false; long long manualExpire=0; bool manualOn=false; {std::lock_guard<std::mutex> lk(g.m); manualOn=g.manual_active; manualExpire=g.manual_expires_ms;} if(manualOn && now_ms()>=manualExpire){ send_bridge_j(0,0); std::lock_guard<std::mutex> lk(g.m); g.manual_active=false; g.manual_cmd_pan=0; g.manual_cmd_tilt=0; g.manual_expires_ms=0; } int effectiveMaxPan=std::max(1,(int)std::lround((sp.max_pan>0?sp.max_pan:cfg.max_pan)*speedScale)); int effectiveMaxTilt=std::max(1,(int)std::lround((sp.max_tilt>0?sp.max_tilt:cfg.max_tilt)*speedScale)); int effectiveMaxAccel=std::max(1,(int)std::lround((sp.max_accel>0?sp.max_accel:cfg.max_accel)*speedScale)); const int effectiveMinPan=sp.min_pan>0?std::max(1,(int)std::lround(sp.min_pan*speedScale)):0; const int effectiveMinTilt=sp.min_tilt>0?std::max(1,(int)std::lround(sp.min_tilt*speedScale)):0; if(active && ok && ts.mode=="TRACKING" && ts.valid){double cx=(ts.l+ts.r)*0.5, cy=(ts.t+ts.b)*0.5; const double nx=cx/std::max(1,cfg.width); const double ny=cy/std::max(1,cfg.height); ex=(nx-cfg.target_x)*2.0; ey=(ny-cfg.target_y)*2.0; auto now=std::chrono::steady_clock::now(); double dt=std::chrono::duration<double>(now-last).count(); last=now; dt=std::max(0.02,std::min(0.2,dt)); double dx=clampd((ex-pex)/dt,-10.0,10.0), dy=clampd((ey-pey)/dt,-10.0,10.0); const double leadSec=std::max(0,std::min(500,sp.ptz_lead_ms))/1000.0; predEx=clampd(ex+dx*leadSec,-1.0,1.0); predEy=clampd(ey+dy*leadSec,-1.0,1.0); const double activeDeadzone=sp.deadzone>0?sp.deadzone:cfg.deadzone; double curvedEx=signed_curve(predEx,activeDeadzone,sp.ptz_curve); double curvedEy=signed_curve(predEy,activeDeadzone,sp.ptz_curve); ix+=signed_curve(ex,activeDeadzone,1.0)*dt; iy+=signed_curve(ey,activeDeadzone,1.0)*dt; pex=ex; pey=ey; basePan=int(std::lround(sp.kp*curvedEx+sp.ki*ix+sp.kd*dx)); baseTilt=int(std::lround(sp.kp*curvedEy+sp.ki*iy+sp.kd*dy)); pan=std::max(-effectiveMaxPan,std::min(effectiveMaxPan,basePan)); tilt=std::max(-effectiveMaxTilt,std::min(effectiveMaxTilt,baseTilt)); if(curvedEx!=0.0 && effectiveMinPan>0 && std::abs(pan)<effectiveMinPan) pan=(curvedEx>0)?effectiveMinPan:-effectiveMinPan; if(curvedEy!=0.0 && effectiveMinTilt>0 && std::abs(tilt)<effectiveMinTilt) tilt=(curvedEy>0)?effectiveMinTilt:-effectiveMinTilt; if(cfg.invert_pan) pan=-pan; if(cfg.invert_tilt) tilt=-tilt; pan=std::max(prevp-effectiveMaxAccel,std::min(prevp+effectiveMaxAccel,pan)); tilt=std::max(prevt-effectiveMaxAccel,std::min(prevt+effectiveMaxAccel,tilt)); prevp=pan; prevt=tilt; sent_stop=false; needSend=true;} else {ix=iy=pex=pey=0; prevp=prevt=0; if(!sent_stop){pan=0;tilt=0; sent_stop=true; needSend=true;} else {std::this_thread::sleep_for(std::chrono::milliseconds(10));}}
    {std::lock_guard<std::mutex> lk(g.m); g.last_tracker_mode=ok?ts.mode:"UNREACHABLE"; g.last_track_id=ts.track_id; g.errx=ex; g.erry=ey; g.pred_errx=predEx; g.pred_erry=predEy; g.cmdp=pan; g.cmdt=tilt; g.mode=(active?"ACTIVE":"IDLE"); g.zoom_ratio=zs.zoom_ratio; g.zoom_focal_px=zs.focal_px; g.zoom_focal_min_px=zs.focal_min_px; g.zoom_focal_max_px=zs.focal_max_px; g.zoom_speed_scale=speedScale; g.base_cmd_pan=basePan; g.base_cmd_tilt=baseTilt; g.scaled_cmd_pan=pan; g.scaled_cmd_tilt=tilt; g.effective_max_pan=effectiveMaxPan; g.effective_max_tilt=effectiveMaxTilt; g.effective_max_accel=effectiveMaxAccel; g.active_profile_idx=sp.profile_idx; g.active_zoom_sample_idx=zs.zoom_sample_idx; g.profile_max_pan=sp.max_pan; g.profile_max_tilt=sp.max_tilt; g.profile_max_accel=sp.max_accel; g.ptz_curve=sp.ptz_curve; g.ptz_lead_ms=sp.ptz_lead_ms; g.speed_profile_source=sr.source; g.speed_profile_left_idx=sr.left_profile_idx; g.speed_profile_right_idx=sr.right_profile_idx; g.speed_profile_t=sr.t; g.zoom_source=zs.source; g.zoom_error=zs.error; if(!ok) g.last_error="tracker_unreachable"; else g.last_error.clear();}
...
static void control_server(){int s=socket(AF_INET,SOCK_STREAM,0); int on=1; setsockopt(s,SOL_SOCKET,SO_REUSEADDR,&on,sizeof(on)); sockaddr_in a{}; a.sin_family=AF_INET;a.sin_port=htons(cfg.control_port); a.sin_addr.s_addr=htonl(INADDR_ANY); bind(s,(sockaddr*)&a,sizeof(a)); listen(s,16); while(g.run){int c=accept(s,nullptr,nullptr); if(c<0) continue; std::string req; char b[4096]; int n=recv(c,b,sizeof(b),0); if(n>0) req.assign(b,n); std::string method=req.substr(0,req.find(' ')); auto p2=req.find(' ',req.find(' ')+1); std::string path=req.substr(req.find(' ')+1,p2-req.find(' ')-1); auto bp=req.find("\r\n\r\n"); std::string body=(bp==std::string::npos)?"":req.substr(bp+4); std::string out="{\"ok\":false}"; int status=200; std::string status_text="OK";
 if(method=="OPTIONS"){status=204; status_text="No Content"; out.clear();}
 ...
 else if(method=="POST" && path=="/api/control/manual_j_pulse"){
  ...
  if(g.enabled.load() && g.mode=="ACTIVE"){ out="{\"ok\":false,\"error\":\"autopilot_active\"}"; }
    ...
    MovementRequest mr; json_num(body,"pan",mr.pan_norm); json_num(body,"tilt",mr.tilt_norm); mr.source=json_str(body,"source"); if(mr.source.empty()) mr.source="web";
    int hold=250; json_int(body,"hold_ms",hold); hold=clampi(hold,50,1000);
    auto res=build_scaled_movement_command(mr); bool ok=send_bridge_j(res.final_pan,res.final_tilt); long long exp=now_ms()+hold;
    { std::lock_guard<std::mutex> lk(g.m); g.manual_active=true; g.manual_last_source=mr.source; g.manual_cmd_pan=res.final_pan; g.manual_cmd_tilt=res.final_tilt; g.manual_profile_idx=res.profile_idx; g.manual_zoom_sample_idx=res.zoom_sample_idx; g.manual_zoom_ratio=res.zoom_ratio; g.manual_expires_ms=exp; g.manual_hold_ms=hold; g.active_profile_idx=res.zoom_sample_idx; g.active_zoom_sample_idx=res.zoom_sample_idx; g.profile_max_pan=res.effective_max_pan; g.profile_max_tilt=res.effective_max_tilt; g.profile_max_accel=res.effective_max_accel; g.speed_profile_source=res.speed_profile_source; g.speed_profile_left_idx=res.left_profile_idx; g.speed_profile_right_idx=res.right_profile_idx; g.speed_profile_t=res.interpolation_t; }
    std::ostringstream os; os<<"{\"ok\":"<<(ok?"true":"false")<<",\"invert_pan\":"<<(cfg.invert_pan?"true":"false")<<",\"invert_tilt\":"<<(cfg.invert_tilt?"true":"false")<<",\"profile_idx\":"<<res.zoom_sample_idx<<",\"zoom_ratio\":"<<res.zoom_ratio<<",\"speed_profile_source\":\""<<res.speed_profile_source<<"\",\"left_profile_idx\":"<<res.left_profile_idx<<",\"right_profile_idx\":"<<res.right_profile_idx<<",\"interpolation_t\":"<<res.interpolation_t<<",\"base_pan\":"<<res.base_pan<<",\"base_tilt\":"<<res.base_tilt<<",\"final_pan\":"<<res.final_pan<<",\"final_tilt\":"<<res.final_tilt<<",\"hold_ms\":"<<hold<<"}"; out=os.str();
  }
 }
 else if(method=="POST" && path=="/api/control/stop"){ send_bridge_j(0,0); {std::lock_guard<std::mutex> lk(g.m); g.manual_active=false; g.manual_cmd_pan=0; g.manual_cmd_tilt=0; g.manual_expires_ms=0;} out="{\"ok\":true}"; }
