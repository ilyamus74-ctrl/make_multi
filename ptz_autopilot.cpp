...
static std::string state_json(){std::lock_guard<std::mutex>lk(g.m); std::ostringstream os; os<<"{\"ok\":true,\"enabled\":"<<(g.enabled?"true":"false")<<",\"mode\":\""<<g.mode<<"\",\"last_tracker_mode\":\""<<g.last_tracker_mode<<"\",\"last_track_id\":"<<g.last_track_id<<",\"err_x\":"<<g.errx<<",\"err_y\":"<<g.erry<<",\"cmd_pan\":"<<g.cmdp<<",\"cmd_tilt\":"<<g.cmdt<<",\"seq\":"<<g.seq.load()<<",\"last_error\":\""<<g.last_error<<"\"}"; return os.str();}

static void control_server(){int s=socket(AF_INET,SOCK_STREAM,0); int on=1; setsockopt(s,SOL_SOCKET,SO_REUSEADDR,&on,sizeof(on)); sockaddr_in a{}; a.sin_family=AF_INET;a.sin_port=htons(cfg.control_port); a.sin_addr.s_addr=htonl(INADDR_LOOPBACK); bind(s,(sockaddr*)&a,sizeof(a)); listen(s,16); while(g.run){int c=accept(s,nullptr,nullptr); if(c<0) continue; std::string req; char b[4096]; int n=recv(c,b,sizeof(b),0); if(n>0) req.assign(b,n); std::string method=req.substr(0,req.find(' ')); auto p2=req.find(' ',req.find(' ')+1); std::string path=req.substr(req.find(' ')+1,p2-req.find(' ')-1); auto bp=req.find("\r\n\r\n"); std::string body=(bp==std::string::npos)?"":req.substr(bp+4); std::string out="{\"ok\":false}";
 if(method=="GET" && path=="/api/autopilot/state") out=state_json();
