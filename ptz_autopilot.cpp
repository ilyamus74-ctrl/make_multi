...
struct TrackerState {std::string mode="IDLE"; int track_id=0; bool valid=false; int l=0,t=0,r=0,b=0; bool predicted_valid=false; double predicted_cx=0,predicted_cy=0;};
...
static bool parse_tracker(const std::string&j,TrackerState&ts){
  ...
  if (json_num(j,"predicted_cx",ts.predicted_cx) && json_num(j,"predicted_cy",ts.predicted_cy)) {
    ts.predicted_valid = ts.predicted_cx >= 0.0 && ts.predicted_cx <= 1.0 && ts.predicted_cy >= 0.0 && ts.predicted_cy <= 1.0;
  }
