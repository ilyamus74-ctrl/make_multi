...
struct SpeedPoint { int step=0; double zoom_ratio=0.0,focal_px=0.0,kp=0,ki=0,kd=0,deadzone=0; int max_pan=0,max_tilt=0,max_accel=0,min_pan=0,min_tilt=0; };
...
static std::string speed_point_to_json(const SpeedPoint& p){ std::ostringstream os; os<<"{\"step\":"<<p.step<<",\"zoom_ratio\":"<<p.zoom_ratio<<",\"focal_px\":"<<p.focal_px<<",\"kp\":"<<p.kp<<",\"ki\":"<<p.ki<<",\"kd\":"<<p.kd<<",\"deadzone\":"<<p.deadzone<<",\"max_pan\":"<<p.max_pan<<",\"max_tilt\":"<<p.max_tilt<<",\"max_accel\":"<<p.max_accel<<",\"min_pan\":"<<p.min_pan<<",\"min_tilt\":"<<p.min_tilt<<"}"; return os.str(); }
static std::vector<SpeedPoint> load_speed_profile_points(){ std::ifstream f(g_speedProfileFile); if(!f) return {}; std::string body((std::istreambuf_iterator<char>(f)), std::istreambuf_iterator<char>()); std::vector<SpeedPoint> pts; size_t pos=0; while(true){ auto stepPos=body.find("\"step\"",pos); if(stepPos==std::string::npos) break; auto objStart=body.rfind('{',stepPos); auto objEnd=body.find('}',stepPos); if(objStart==std::string::npos||objEnd==std::string::npos) break; std::string obj=body.substr(objStart,objEnd-objStart+1); SpeedPoint p; if(!json_int(obj,"step",p.step)){ pos=objEnd+1; continue; } json_num(obj,"zoom_ratio",p.zoom_ratio); json_num(obj,"focal_px",p.focal_px); json_num(obj,"kp",p.kp); json_num(obj,"ki",p.ki); json_num(obj,"kd",p.kd); json_num(obj,"deadzone",p.deadzone); json_int(obj,"max_pan",p.max_pan); json_int(obj,"max_tilt",p.max_tilt); json_int(obj,"max_accel",p.max_accel); json_int(obj,"min_pan",p.min_pan); json_int(obj,"min_tilt",p.min_tilt); pts.push_back(p); pos=objEnd+1; } return pts; }
static bool save_speed_profile_points(const std::vector<SpeedPoint>& pts){ std::ofstream f(g_speedProfileFile); if(!f) return false; f<<"{\n  \"method\": \"manual_ptz_zoom_speed_profile\",\n  \"version\": 1,\n  \"points\": [\n"; for(size_t i=0;i<pts.size();++i){ f<<"    "<<speed_point_to_json(pts[i]); if(i+1<pts.size()) f<<","; f<<"\n"; } f<<"  ]\n}\n"; return true; }
...
static void control_server(){int s=socket(AF_INET,SOCK_STREAM,0); int on=1; setsockopt(s,SOL_SOCKET,SO_REUSEADDR,&on,sizeof(on)); sockaddr_in a{}; a.sin_family=AF_INET;a.sin_port=htons(cfg.control_port); a.sin_addr.s_addr=htonl(INADDR_ANY); bind(s,(sockaddr*)&a,sizeof(a)); listen(s,16); while(g.run){int c=accept(s,nullptr,nullptr); if(c<0) continue; std::string req; char b[4096]; int n=recv(c,b,sizeof(b),0); if(n>0) req.assign(b,n); std::string method=req.substr(0,req.find(' ')); auto p2=req.find(' ',req.find(' ')+1); std::string path=req.substr(req.find(' ')+1,p2-req.find(' ')-1); auto bp=req.find("\r\n\r\n"); std::string body=(bp==std::string::npos)?"":req.substr(bp+4); std::string out="{\"ok\":false}"; int status=200; std::string status_text="OK";
 ...
 else if(method=="GET" && path=="/api/autopilot/speed_profile"){ out=speed_profile_json(load_speed_profile_points()); }
 else if(method=="POST" && path=="/api/autopilot/speed_profile/save_point"){ int step=0; double zoom_ratio=0.0,focal_px=0.0; json_int(body,"step",step); json_num(body,"zoom_ratio",zoom_ratio); json_num(body,"focal_px",focal_px); auto pts=load_speed_profile_points(); SpeedPoint np; np.step=step; np.zoom_ratio=zoom_ratio; np.focal_px=focal_px; np.kp=cfg.kp; np.ki=cfg.ki; np.kd=cfg.kd; np.deadzone=cfg.deadzone; np.max_pan=cfg.max_pan; np.max_tilt=cfg.max_tilt; np.max_accel=cfg.max_accel; np.min_pan=cfg.min_pan; np.min_tilt=cfg.min_tilt; bool replaced=false; for(auto &p:pts){ if(p.step==step){ p=np; replaced=true; break; } } if(!replaced) pts.push_back(np); save_speed_profile_points(pts); std::ostringstream os; os<<"{\"ok\":true,\"saved\":true,\"points\":"<<pts.size()<<"}"; out=os.str(); }
 else if(method=="POST" && path=="/api/autopilot/speed_profile/apply_nearest"){ double zr=0.0; json_num(body,"zoom_ratio",zr); auto pts=load_speed_profile_points(); if(pts.empty()){ out="{\"ok\":false,\"error\":\"no_points\"}"; } else { size_t best=0; double bd=1e9; for(size_t i=0;i<pts.size();++i){ double d=std::abs(pts[i].zoom_ratio-zr); if(d<bd){bd=d; best=i;} } auto &p=pts[best]; cfg.kp=p.kp; cfg.ki=p.ki; cfg.kd=p.kd; cfg.deadzone=p.deadzone; cfg.max_pan=p.max_pan; cfg.max_tilt=p.max_tilt; cfg.max_accel=p.max_accel; cfg.min_pan=p.min_pan; cfg.min_tilt=p.min_tilt; clamp_config(); save_ptz_config(); out=std::string("{\"ok\":true,\"applied\":true,\"point\":")+speed_point_to_json(p)+"}"; } }
 else if(method=="POST" && path=="/api/autopilot/speed_profile/build"){ auto pts=load_speed_profile_points(); std::sort(pts.begin(),pts.end(),[](const SpeedPoint&a,const SpeedPoint&b){return a.zoom_ratio<b.zoom_ratio;}); bool ok=save_speed_profile_points(pts); std::ostringstream os; os<<"{\"ok\":"<<(ok?"true":"false")<<",\"points\":"<<pts.size()<<"}"; out=os.str(); }
