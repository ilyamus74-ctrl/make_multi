...
server.Get("/api/calibration_new/video", [this](const Request&, Response& res) {
    ...
    bool restartCaptureStream() {
        ...
        fprintf(stderr,
                "Calibration stream restarted after DQBUF error (%d/%d attempts)\n",
        ...
        if (ioctl(cam_fd, VIDIOC_DQBUF, &buf) < 0) {
            ...
            if (err == EIO || err == EPIPE) {
                if (stream_restart_attempts_ >= kMaxStreamRestarts ||
                    ...
                    fprintf(stderr,
                            "Calibration camera: DQBUF failed with %s after %d attempts\n",
    ...
    void cameraLoop() {
        ...
        while (cam_running.load()) {
            ...
            if (cap_status == CaptureStatus::kFatal) {
                fprintf(stderr, "Fatal calibration capture error, stopping process\n");
                raise(SIGTERM);
                break;
            }
