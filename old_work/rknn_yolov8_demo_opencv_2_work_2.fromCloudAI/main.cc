...
int main(int argc, char **argv)
{
    ...
    std::unique_ptr<unsigned char, decltype(&free)> src_guard(nullptr, &free);
    do
    {
        ...
        src_guard.reset(src_image.virt_addr);
        ...
        for (int i = 0; i < od_results.count; i++)
        {
            ...
            //printf("ID:%d %s @ (%d %d %d %d) %.3f\n", det_result->id, coco_cls_to_name(det_result->cls_id), det_result->prop * 100);
