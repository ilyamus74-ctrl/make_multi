...
struct CalibrationParams {
    ...
    int min_frames = 15;       // минимум качественных кадров для калибровки (моно)
    int min_stereo_matches = 12; // минимум совпадающих кадров для стерео-калибровки
    ...
    int time_tolerance_ms = 33; // допустимое расхождение таймстемпов при синхронизации
    bool enable_multithreading = false; // использовать многопоточную обработку видео
    float stereo_pose_max_center_diff_vertical = 0.05f;    // максимум по нормированной разнице центров по вертикали (Y)
    float stereo_pose_max_center_diff_horizontal = 0.35f;  // максимум по нормированной разнице центров по горизонтали (X)
    float stereo_pose_max_normalized_span_ratio = 0.3f; // максимум по относительному нормализованному размеру
    float stereo_pose_max_tilt_diff = 10.0f;    // максимум по углу наклона в градусах
...
struct CameraCalibrationResult {
    ...
    std::string mode;                   // normalized mode identifier ("", "wide_angle", "zoom", ...)
...
struct BoardPose {
    ...
    float scale = 0.0f;                       // Legacy scale value (now stores normalized diagonal span when available)
    float pixel_span_horizontal = 0.0f;       // Average horizontal span in pixels
    float pixel_span_vertical = 0.0f;         // Average vertical span in pixels
    float pixel_span_diagonal = 0.0f;         // Combined diagonal span in pixels
    float normalized_span_horizontal = 0.0f;  // Average horizontal span in normalized camera space
    float normalized_span_vertical = 0.0f;    // Average vertical span in normalized camera space
    float normalized_span_diagonal = 0.0f;    // Combined diagonal span in normalized camera space
};
...
public:
    ...
    struct RecordingInventory {
        struct VideoInfo {
            ...
            std::vector<std::string> capture_group;
            std::filesystem::path path;
            std::time_t last_modified{0};
            uintmax_t file_size{0};
        };

        std::set<std::string> mono_cameras;
        std::vector<std::vector<std::string>> stereo_pairs;
    ...
    // Общие вспомогательные функции для построения идентификатора профиля калибровки
    static std::string normalizeModeName(const std::string& mode);
    static std::string makeProfileKey(const std::string& camera_id, const std::string& mode);
    static std::pair<std::string, std::string> splitProfileKey(const std::string& profile_key);
    ...
    RecordingInventory getRecordingInventory();
    ...
    void setLifecycleCallbacks(LifecycleCallback on_begin, LifecycleCallback on_finish);
    ...
    bool getCameraMatrix(const std::string& camera_id, const std::string& mode,
                         cv::Mat& camera_matrix, cv::Mat& dist_coeffs) const;
    bool getCameraMatrixForProfile(const std::string& profile_key,
                                   cv::Mat& camera_matrix, cv::Mat& dist_coeffs) const;
    bool getStereoParams(const std::string& camera_pair, cv::Mat& R, cv::Mat& T, cv::Mat& Q) const;

    std::filesystem::path getResultsPath() const { return results_dir_; }

private:
    struct VideoFile {
        std::string camera_id;
        std::string mode;            // normalized mode extracted from filename (may be empty)
        std::string capture_type;    // capture tag derived from filename prefix ("mono", "stereo", ...)
        std::vector<std::string> capture_group; // additional cameras referenced in prefix (if any)
        std::filesystem::path path;
        std::time_t last_modified;
        uintmax_t file_size;
        std::string profileKey() const {
            return CalibrationWatcher::makeProfileKey(camera_id, mode);
        }
    };
...
private:
    ...
    LifecycleCallback pre_calibration_callback_;
    LifecycleCallback post_calibration_callback_;
    ...
    bool invokePreCalibrationCallback();
    void invokePostCalibrationCallback();
 ...
 static BoardPose analyzeBoardPose(const std::vector<cv::Point2f>& corners,
    ...
    void analyzeTimestampAlignment(const std::vector<VideoFile>& video_files,
    ...
    bool performStereoCalibration(const std::string& cam_first, const std::string& cam_second,
    ...
    bool ensureConsistentCornerOrder(std::vector<cv::Point2f>& corners,
                                     const cv::Size& pattern_size,
                                     double timestamp,
    ...
    std::filesystem::path cameraModeRootDir(const std::string& camera_id, const std::string& mode) const;
    std::filesystem::path cameraModeImagesDir(const std::string& camera_id, const std::string& mode) const;


    // Оценка и хранение временных смещений между камерами
    void computeCameraOffsets(const std::vector<std::string>& camera_ids,
                              const CalibrationParams& params,
                              const std::map<std::string, std::vector<double>>& timestamp_index);
    bool loadExternalCameraOffsets(std::map<std::string, double>& offsets) const;
    std::vector<double> loadChessboardTimestamps(const std::string& camera_id) const;
    double estimateOffsetFromTimestamps(const std::vector<double>& reference,
                                        const std::vector<double>& target,
                                        double tolerance) const;
    double estimateOffsetFromIndex(const std::string& reference_camera,
                                   const std::string& target_camera,
                                   const std::map<std::string, std::vector<double>>& timestamp_index) const;
    
    mutable std::mutex camera_offsets_mutex_;
    std::map<std::string, double> camera_time_offsets_;
};
...
private:
    ...
    const CameraCalibrationResult* findMonoCalibration(const std::string& requested) const;
};
