...
CameraSchemeManager::CameraSchemeManager()
    : current_scheme_(SchemeType::UNKNOWN), is_calibrated_(false) {
...
bool CameraSchemeManager::updateSphereTracking(const std::map<std::string, std::vector<Point2D>>& detections) {
    ...
    // Обновляем трекинг для каждой камеры
    for (const auto& [camera_id, points] : detections) {
        ...
        // Для каждой детекции создаем или обновляем трек
        for (size_t i = 0; i < points.size(); ++i) {
            std::string object_id = camera_id + "_obj_" + std::to_string(i);
            
            SphereTrackingResult& track = active_tracks_[object_id];
            track.object_id = object_id;
            track.visible_cameras.clear();
            track.visible_cameras.push_back(camera_id);
            track.primary_camera = camera_id;
            track.confidence = 0.8; // Базовая уверенность
            
            // TODO: Реализовать триангуляцию для определения world_position
            // Пока используем упрощенную проекцию
            track.world_position.x = points[i].x - camera->width / 2.0;
            track.world_position.y = points[i].y - camera->height / 2.0;
            track.world_position.z = 0.0;
        }
    }
...
CameraSchemeManager::SphereStats CameraSchemeManager::getSphereStats() const {
    SphereStats stats{};
    stats.total_objects_tracked = active_tracks_.size();
    
    // TODO: Реализовать сбор реальной статистики
    stats.successful_handoffs = 0;
    stats.failed_handoffs = 0;
    stats.average_tracking_accuracy = 0.85;
    
    // Статистика использования камер
