    ...
    try {
        ...
        for (size_t i = 0; i < total_videos; ++i) {
            ...
            if (!isVideoComplete(video)) {
                ...
                if (indexVideoPTS(video, timestamps) && !timestamps.empty()) {
                    timestamp_index.emplace(video.camera_id, std::move(timestamps));
                    indexed = true;
                }
            }
       ...
       successful_cameras.reserve(processed_cameras.size());
        ...
        if (successful_cameras.size() >= 2) {
            ...
            logMessage("Not enough cameras passed mono calibration for stereo stage");
        }
...
bool CalibrationWatcher::indexVideoPTS(const VideoFile& video, std::vector<double>& timestamps) {
    ...
    if (!timestamps.empty()) {
        logMessage("Indexed " + std::to_string(timestamps.size()) +
                   " timestamps for camera " + video.camera_id);
        return true;
    }

    logMessage("No timestamps indexed for camera " + video.camera_id +
...
std::vector<std::string> CalibrationWatcher::extractFramesFromAllVideos(
    ...
    for (size_t i = 0; i < total_videos && !should_stop_.load(); ++i) {
        ...
        float progress_before = progress_start;
        if (total_videos > 0) {
            progress_before += progress_range *
                                static_cast<float>(i) / static_cast<float>(total_videos);
        }

        std::ostringstream stage_message;
        stage_message << "Detecting chessboard patterns (" << (i + 1) << "/"
                      << total_videos << ") for camera " << video.camera_id;
        const std::string stage_text = stage_message.str();
        updateStatus(stage_text, progress_before);
        ...
        if (!isVideoComplete(video)) {
            ...
            auto index_it = timestamp_index.find(video.camera_id);
        ...
        float progress_after = progress_start;
        if (total_videos > 0) {
            progress_after += progress_range *
                              static_cast<float>(i + 1) / static_cast<float>(total_videos);
        }

        std::ostringstream completed_message;
        completed_message << stage_text;
        if (skipped) {
        ...
        updateStatus(completed_message.str(), progress_after);
    }
...
bool CalibrationWatcher::extractFramesFromVideo(const VideoFile& video,
    ...
    if (!using_indexed_timestamps) {
        logMessage("PTS index unavailable for camera " + video.camera_id +
