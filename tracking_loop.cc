    ...
    for (auto& t : tracks) {
        ...
        if (best != -1) {
            ...
            if (use_grayscale_tracking) {
                ...
                t.has_grayscale_descriptor = true;
                t.has_color_descriptor = false;
            } else {
                avgColor(img, det.box, t.color);
                t.has_color_descriptor = true;
                t.has_grayscale_descriptor = false;
            }
...
bool SimpleTracker::getDescriptor(int id, TrackDescriptor& out) const {
    std::lock_guard<std::mutex> lock(tracker_mutex_);

    auto fill = [&out](const Track& track) {
        out.has_color = track.has_color_descriptor;
        if (track.has_color_descriptor) {
            out.color[0] = track.color[0];
            out.color[1] = track.color[1];
            out.color[2] = track.color[2];
        }
        out.has_grayscale = track.has_grayscale_descriptor;
        if (track.has_grayscale_descriptor) {
            out.grayscale_intensity = track.grayscale_intensity;
            for (int i = 0; i < 4; ++i) {
                out.texture_features[i] = track.texture_features[i];
            }
        }
    };
...
nlohmann::json formatDetectionResults(object_detect_result_list* results, int img_w, int img_h, const SimpleTracker& tracker) {
    ...
    for (int i = 0; i < results->count; ++i) {
        ...
        if (tracker.getDescriptor(d->track_id, descriptor)) {
            nlohmann::json desc_json;
            if (descriptor.has_color) {
                desc_json["mode"] = "color";
                desc_json["rgb"] = {descriptor.color[0], descriptor.color[1], descriptor.color[2]};
            }
            if (descriptor.has_grayscale) {
                // Preserve mode if both descriptors are available
                if (!descriptor.has_color) {
                    desc_json["mode"] = "grayscale";
                } else {
                    desc_json["mode"] = "mixed";
                }
                desc_json["intensity"] = descriptor.grayscale_intensity;
                desc_json["texture"] = {descriptor.texture_features[0], descriptor.texture_features[1],
                                         descriptor.texture_features[2], descriptor.texture_features[3]};
            }
            if (!desc_json.empty()) {
                j["descriptor"] = std::move(desc_json);
            }
        }
        arr.push_back(j);
    }
