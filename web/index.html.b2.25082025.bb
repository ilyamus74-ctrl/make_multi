...
<body>
  <div>
    ...
    <button id="tabbtn-stereo">stereo settings</button>
  </div>
  ...
  <div id="tab-settings" class="tab">
    <h3>Configured cameras <button onclick="showStereoRuntime()">Stereo Detection & Tracking</button></h3>
    <div id="configured"></div>

    <!-- Persistent advanced-settings panel -->
    <div id="adv-settings" data-loaded="0"></div>
    <div id="stereo-runtime" style="display:none;position:fixed;top:10%;left:10%;background:#fff;border:1px solid #888;padding:10px;"></div>
  ...
  <div id="tab-stereo-settings" class="tab">
    <h3>Stereo Settings</h3>
    <div id="stereo-settings"></div>
  </div>
  ...
  <script>
    async function showTab(id){
      ...
      if(id==='tab-main'){
        ...
        if(id==='tab-settings'){
          ...
          currentAdvId=null;
          advDirty=false;
        }else if(id==='tab-stereo-settings'){
          await showStereoSettings();
        }
      }
    }
    ...
    document.getElementById('tabbtn-stereo').onclick=()=>showTab('tab-stereo-settings');
    */
    ...
    document.getElementById('tabbtn-main').onclick=()=>gotoTab('main');
    document.getElementById('tabbtn-settings').onclick=()=>gotoTab('settings');
    ...
    async function showStereoSettings(){
      ...
      try{ await fetch('/api/calib/prepare',{method:'POST'}); }catch(e){}
      let cams=[];
      try{
      ...
      async function refreshButtons(){
        let st={};
        try{
          const resp=await fetch('/api/calib/status',{cache:'no-store'});
          if(!resp.ok){
          ...
          console.error('refreshButtons network error:', e);
        ...
        save.onclick=async()=>{
          try{
            const resp=await fetch('/api/calib/setup',{
              method:'POST',
            ...
            if(!resp.ok){
              ...
              alert('Calibration folder created');
            }
          }catch(e){
            console.error('save setup network error:', e);
            alert('Network error: '+e);
          }
      ...
      mono.onclick=async()=>{
        mono.disabled=true;
        try{
          const resp=await fetch('/api/calib/mono',{
            method:'POST',
            headers:{'Content-Type':'application/json'},
            body:JSON.stringify({camera:sel.value})
          });
          ...
          console.error('mono calibration network error:', e);
          alert('Network error: '+e);
        }
        await refreshButtons();
      };
      ...
      stereo.onclick=async()=>{
        ...
        try{
          const resp=await fetch('/api/calibrate/stereo-auto',{
            method:'POST',
            headers:{'Content-Type':'application/json'},
            body:JSON.stringify({camera:sel.value})
          });
          ...
          console.error('stereo calibration network error:', e);
          alert('Network error: '+e);
        }
        await refreshButtons();
      };
 ...
 async function showStereoRuntime(){
      const panel=document.getElementById('stereo-runtime');
      panel.style.display='block';
      panel.innerHTML='Loading...';
      let cfg={range:0,tracking:false};
      try{
      ...
      panel.appendChild(document.createTextNode('detection range:'));
      const range=document.createElement('input');
      range.type='number';
      range.value=cfg.range||0;
      range.id='rt-range';
      panel.appendChild(range);
      panel.appendChild(document.createElement('br'));

      panel.appendChild(document.createTextNode('enable tracking:'));
      const tracking=document.createElement('input');
      tracking.type='checkbox';
      tracking.checked=!!cfg.tracking;
      tracking.id='rt-track';
      ...
      save.onclick=async()=>{
        try{
          const resp=await fetch('/api/runtime/config',{
            method:'POST',
            headers:{'Content-Type':'application/json'},
            body:JSON.stringify({range:parseInt(range.value),tracking:tracking.checked})
          });
          if(!resp.ok){
            const txt=await resp.text();
            alert(`Error ${resp.status}: ${txt}`);
          ...
          console.error('runtime config network error:', e);
          alert('Network error: '+e);
        }
      };
      panel.appendChild(save);
...
// ----- Main tab detection stream -----
    ...
    async function updateStatus(){
      try{
        ...
        previewEnabled=s.preview;
      }catch(e){
        console.error('reloadConfig network error:', e);
      }
    }

    setInterval(()=>{updateStatus().then(()=>refresh());},1000);
    /*updateStatus().then(()=>refresh());*/
    fetch('/api/config',{cache:'no-store'}).then(r=>r.json()).then(c=>{previewEnabled=c.preview_enabled; refresh();});
    const initTab=new URLSearchParams(location.search).get('tab')||'main';
    showTab('tab-'+initTab);
  </script>
</body>
</html>
