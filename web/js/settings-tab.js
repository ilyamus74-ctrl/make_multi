...
window.CameraApp.SettingsTab = {
    ...
    setupEventListeners() {
        ...
        const managerDebugToggle = document.getElementById('manager-debug-toggle');
        if (managerDebugToggle) {
    ...
    handleDynamicEvents(event) {
        ...
        const settingsButton = target.closest('[data-camera-settings]');
        if (settingsButton) {
            const cameraId = settingsButton.getAttribute('data-camera-settings');
            this.showAdvancedSettings(cameraId);
        }

        // Camera delete button
        const deleteButton = target.closest('[data-camera-delete]');
        if (deleteButton) {
            const cameraId = deleteButton.getAttribute('data-camera-delete');
    ...
    async handleManagerDebugToggle(event) {
        const toggle = event.target;
        const previous = window.CameraApp.State.managerDebugEnabled;
        const enabled = toggle.checked;

        toggle.disabled = true;
        try {
            await window.CameraApp.API.setManagerDebugLogging(enabled);
            this.updateManagerDebugToggle(enabled);
            window.CameraApp.UI.showToast(
                `Manager debug logging ${enabled ? 'enabled' : 'disabled'}`,
                'success'
            );
        } catch (error) {
            console.error('Failed to update manager debug logging:', error);
            toggle.checked = previous;
            window.CameraApp.UI.showToast(
                'Failed to update manager debug logging',
                'danger'
            );
        } finally {
    ...
    async loadData() {
        try {
            ...
            this.updateManagerDebugToggle(!!config.manager_debug_enabled);
    ...
    updateManagerDebugToggle(enabled) {
        const toggle = document.getElementById('manager-debug-toggle');
