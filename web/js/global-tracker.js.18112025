...
window.CameraApp.GlobalTracker = {
    ...
    selectObject(objectId) {
        ...
        if (wasSelected) {
            // Deselecting clears highlights on all cameras
            this.highlightObjectOnStreams(-1);
        } else {
            const objects = window.CameraApp.State.lastGlobalObjects || [];
            const selectedObject = objects.find(obj => obj.id === objectId);

            if (!selectedObject) {
                console.warn('Selected object not found for highlighting:', objectId);
                this.highlightObjectOnStreams(-1);
            } else {
                this.highlightObjectOnStreams(selectedObject);
            }
        }
    ...
    async highlightObjectOnStreams(selection) {
        // When selection is -1, clear highlights on all cameras
        if (selection === -1) {
            const clearPromises = window.CameraApp.State.mainCameras.map(async (camera) => {
                try {
                    await window.CameraApp.API.highlightObject(camera.det_port, -1);
                } catch (error) {
                    console.error(`Failed to clear highlight on camera ${camera.id}:`, error);
                }
            });
        ...
        if (!selection || !Array.isArray(selection.cameras)) {
            console.warn('No camera mappings available for selection:', selection);
            return;
        }

        const highlightPromises = selection.cameras.map(async (camEntry) => {
            if (!camEntry || camEntry.track_id == null || camEntry.track_id < 0) {
                return;
            }

            const camera = window.CameraApp.State.mainCameras.find(c => c.id === camEntry.camera);
            if (!camera || camera.det_port == null) {
                return;
            }

            try {
                await window.CameraApp.API.highlightObject(camera.det_port, camEntry.track_id);
            } catch (error) {
