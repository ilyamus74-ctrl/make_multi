...
#define HAVE_OPENCV_ARUCO 1
#else
#define HAVE_OPENCV_ARUCO 0
#endif
...
static ZoomMarkerMeasurement measure_zoom_markers_apriltag(const cv::Mat& gray, const ZoomAprilTagCalibParams& p) {
  ...
  for (const auto& c : corners) {
    ...
    const double w = 0.5 * (cv::norm(c[0] - c[1]) + cv::norm(c[2] - c[3]));
    if (w <= 0.0) continue;
    if (m.tags_found == 0) m.near_px = w;
    else if (w > m.near_px) { m.far_px = m.near_px; m.near_px = w; }
    else m.far_px = std::max(m.far_px, w);
    m.tags_found++;
  }
  if (m.tags_found <= 0) { m.message = "apriltag_not_found"; return m; }
  if (m.near_px > 0.0) m.focal_px += m.near_px * (p.near_distance_mm / p.tag_size_mm);
  if (m.far_px > 0.0) m.focal_px += m.far_px * (p.far_distance_mm / p.tag_size_mm);
  m.focal_px /= (m.far_px > 0.0 ? 2.0 : 1.0);
  m.ok = m.focal_px > 0.0; m.confidence = m.tags_found >= 2 ? 1.0 : 0.5;
#else
  (void)gray; (void)p;
