...
#define APP_HAVE_OPENCV_ARUCO 1
#else
#define APP_HAVE_OPENCV_ARUCO 0
#endif
...
struct ZoomMasterProfile {
  ...
  double focal_min_px = 0.0;
  double focal_max_px = 0.0;
  std::vector<ZoomProfilePoint> points;
};
...
static std::mutex g_zoomRuntimeMtx;
static std::string g_zoomRuntimeStateFile = "zoom_runtime_state.json";
static std::atomic<int> g_zoomStepsSinceHome{0};
static std::atomic<int64_t> g_zoomLastHomeMs{0};
static std::atomic<int64_t> g_zoomLastMoveMs{0};
static std::atomic<bool> g_zoomMoveBusy{false};
...
static std::atomic<int> g_zoomDriftMinSteps{8};
static std::atomic<bool> g_zoomDriftEnable{true};
...
static int g_zoomCalibProgressIdx = 0;
...
static bool send_zoom_stop_burst_via_bridge_ws() {
...
static std::string zoom_calib_settings_json(const ZoomAprilTagCalibParams& pIn) {
  ...
  os << "{"
     ...
     << ",\"anchor_tag_id\":" << p.anchor_tag_id
     << ",\"anchor_distance_mm\":" << p.anchor_distance_mm
     << ",\"samples\":" << p.samples
...
static bool load_zoom_calib_settings(ZoomAprilTagCalibParams& p, int cmdMaxZoom) {
  ...
  if (extract_json_int_field(body, "anchor_tag_id", &i)) p.anchor_tag_id = i;
  else if (extract_json_int_field(body, "near_tag_id", &i)) p.anchor_tag_id = i;
  if (extract_json_double_field(body, "anchor_distance_mm", &d)) p.anchor_distance_mm = d;
  else if (extract_json_double_field(body, "near_distance_mm", &d)) p.anchor_distance_mm = d;
...
static bool extract_json_double_field(const std::string& json, const std::string& key, double* out) {
  ...
  char* endp = nullptr;
  const double v = std::strtod(json.c_str() + valPos, &endp);
  if (endp == json.c_str() + valPos) return false;
  *out = v;
  return true;
}

static ZoomMarkerMeasurement measure_zoom_markers_apriltag(const cv::Mat& gray, const ZoomAprilTagCalibParams& p) {
  ...
  for (size_t i = 0; i < corners.size(); ++i) {
    m.tags_found++;
    if (i < ids.size()) m.ids.push_back(ids[i]);
    if (p.anchor_tag_id >= 0) {
      if (i < ids.size() && ids[i] == p.anchor_tag_id) anchorIdx = static_cast<int>(i);
    } else if (anchorIdx < 0) {
      anchorIdx = static_cast<int>(i);
    }
  }
  if (m.tags_found <= 0) { m.message = "apriltag_not_found"; return m; }
  if (anchorIdx >= 0 && static_cast<size_t>(anchorIdx) < corners.size()) {
    const auto& c = corners[anchorIdx];
    if (c.size() >= 4) {
      m.anchor_px = 0.5 * (cv::norm(c[0] - c[1]) + cv::norm(c[2] - c[3]));
      m.anchor_found = (m.anchor_px > 0.0);
      m.anchor_id = (anchorIdx < static_cast<int>(ids.size())) ? ids[anchorIdx] : -1;
      if (m.anchor_found) {
        m.focal_px = m.anchor_px * (p.anchor_distance_mm / p.tag_size_mm);
      }
    }
  }
  m.ok = m.anchor_found && m.focal_px > 0.0;
  m.confidence = m.anchor_found ? 1.0 : 0.0;
  if (!m.anchor_found) m.message = "apriltag_anchor_not_found";
#else
  (void)gray; (void)p;
  m.message = "apriltag_detector_unavailable";
#endif
  return m;
}
...
static bool run_zoom_apriltag_table_calibration(const Opts& o, const ZoomAprilTagCalibParams& params) {
  ...
  const bool prevDetect = g_detectEnabled.load();
  g_detectEnabled = false;
  {
    std::lock_guard<std::mutex> slk(g_zoomCalibStatusMtx);
    g_zoomCalibLastStatus = "running";
    g_zoomCalibLastMessage = "apriltag_zoom_table_running";
    g_zoomCalibLastRunAt = now_utc_iso8601();
  }
  ...
  int validRowsRaw = 0;
  int tagsTotal = 0;
  int anchorMissWithTags = 0;
  for (const auto& r : rows) {
    tagsTotal += r.tags_found;
    if (r.tags_found > 0 && !r.anchor_found) anchorMissWithTags++;
    if (r.anchor_found && r.focal_px > 0.0) {
  ...
  for (size_t i = 0; i < rows.size(); ++i) if (rows[i].anchor_found && rows[i].focal_px > 0.0) validIdx.push_back(i);
  std::sort(validIdx.begin(), validIdx.end(), [&](size_t a, size_t b){ return rows[a].focal_px < rows[b].focal_px; });
  std::vector<size_t> cleanIdx;
  for (size_t k = 0; k < validIdx.size(); ++k) {
    if (cleanIdx.empty()) { cleanIdx.push_back(validIdx[k]); continue; }
    const double prevF = rows[cleanIdx.back()].focal_px;
    const double curF = rows[validIdx[k]].focal_px;
    const double thr = std::max(25.0, prevF * 0.03);
    if (std::abs(curF - prevF) >= thr) cleanIdx.push_back(validIdx[k]);
  }
  int validRows = static_cast<int>(cleanIdx.size());
  minF = 1e9; maxF = 0.0;
  for (size_t i : cleanIdx) { minF = std::min(minF, rows[i].focal_px); maxF = std::max(maxF, rows[i].focal_px); }

  const bool hasFocalRange = (validRows > 0 && maxF > minF);
  const bool noTags = (tagsTotal == 0);
  const bool tooFewRows = (validRows < 4);
  ...
  if (noTags) { calibStatus = "failed"; calibMessage = "apriltag_anchor_not_found"; }
  else if (anchorMissWithTags == static_cast<int>(rows.size())) { calibStatus = "failed"; calibMessage = "apriltag_anchor_not_found"; }
  ...
  for (size_t i = 0; i < rows.size(); ++i) {
    ...
    os << "{\"idx\":" << i << ",\"profile_idx\":" << profileIdx << ",\"zoom_ratio\":" << std::fixed << std::setprecision(3) << zr
       ...
       << ",\"tags_found\":" << rows[i].tags_found << ",\"ids\":[";
  ...
  std::ostringstream distName; distName << (params.anchor_distance_mm / 1000.0);
  std::string ds = distName.str(); std::replace(ds.begin(), ds.end(), '.', '_');
  while (!ds.empty() && ds.back() == '0') ds.pop_back();
  if (!ds.empty() && ds.back() == '_') ds.pop_back();
  std::ofstream out2(std::string("zoom_apriltag_profile_anchor_") + ds + "m.json", std::ios::trunc); out2 << g_zoomAprilTagProfileJson;
  {
    std::lock_guard<std::mutex> slk(g_zoomCalibStatusMtx);
...
static bool move_zoom_to_ratio(double targetRatio) { g_zoomRatio = std::max(0.0, std::min(1.0, targetRatio)); return true; }

static std::vector<std::string> extract_json_object_array(const std::string& json, const std::string& key);

static bool load_zoom_master_profile() {
  ZoomMasterProfile next;
  std::ifstream in(g_zoomMasterProfileFile);
  if (!in.good()) {
  ...
  next.status = extract_json_string_field(payload, "status");
  ...
  std::sort(next.points.begin(), next.points.end(), [](const ZoomProfilePoint& a, const ZoomProfilePoint& b){ return a.profile_idx < b.profile_idx; });
  next.loaded = (next.focal_min_px > 0.0 && next.focal_max_px >= next.focal_min_px);
  if (!next.loaded && next.points.size() >= 2) {
    next.focal_min_px = next.points.front().focal_px;
    next.focal_max_px = next.points.back().focal_px;
    next.loaded = next.focal_min_px > 0.0 && next.focal_max_px >= next.focal_min_px;
  }
...
static double zoom_profile_focal_for_ratio(double ratio) {
  ratio = std::max(0.0, std::min(1.0, ratio));
  std::lock_guard<std::mutex> lk(g_zoomMasterProfileMtx);
  const auto& p = g_zoomMasterProfile;
  if (!p.loaded) return 0.0;
  if (p.points.size() >= 2) {
    if (ratio <= p.points.front().zoom_ratio) return p.points.front().focal_px;
    if (ratio >= p.points.back().zoom_ratio) return p.points.back().focal_px;
    for (size_t i = 1; i < p.points.size(); ++i) {
      if (ratio <= p.points[i].zoom_ratio) {
        const auto& a = p.points[i-1]; const auto& b = p.points[i];
        const double d = std::max(1e-9, b.zoom_ratio - a.zoom_ratio);
        const double t = (ratio - a.zoom_ratio) / d;
        return a.focal_px + t * (b.focal_px - a.focal_px);
      }
    }
  }
  if (p.focal_max_px > p.focal_min_px && p.focal_min_px > 0.0) return p.focal_min_px + ratio * (p.focal_max_px - p.focal_min_px);
  return 0.0;
}


static std::string json_escape(const std::string& s);
static int64_t now_ms(){ return (int64_t)std::chrono::duration_cast<std::chrono::milliseconds>(std::chrono::system_clock::now().time_since_epoch()).count(); }
static int zoom_cmd_abs(){ std::lock_guard<std::mutex> lk(g_zoomCalibMtx); return std::max(1, g_zoomCalibSettings.cmd_abs); }
static int zoom_impulse_ms(){ std::lock_guard<std::mutex> lk(g_zoomCalibMtx); return std::max(20, g_zoomCalibSettings.impulse_ms); }
static int zoom_wide_hold_ms(){ std::lock_guard<std::mutex> lk(g_zoomCalibMtx); return std::max(300, g_zoomCalibSettings.wide_hold_ms); }
static int zoom_wide_cmd(){ std::lock_guard<std::mutex> lk(g_zoomCalibMtx); return ((g_zoomCalibSettings.wide_cmd_sign<0)?-1:1)*std::max(1,g_zoomCalibSettings.cmd_abs); }
...
static bool zoom_move_steps(int delta, std::string* err){ int steps=std::abs(delta); int cmd=delta>0?zoom_tele_cmd():zoom_wide_cmd(); for(int i=0;i<steps;++i){ if(!zoom_send_stream_cmd(cmd,zoom_impulse_ms(),err)) return false; std::this_thread::sleep_for(std::chrono::milliseconds(80)); } return true; }
...
static void handle_client(int cfd, const Opts& o) {
  ...
  if (path == "/api/zoom_calibration/clear_profiles") {
    if (method != "POST") {
  ...
  if (path == "/api/zoom/state") {
    if (method == "GET") {
      double zoomRatio = g_zoomRatio.load();
      double zoomConf = g_zoomConfidence.load();
      std::string zoomSource;
      {
        std::lock_guard<std::mutex> lk(g_zoomMasterProfileMtx);
        zoomSource = g_zoomSource;
      }
      const double focalPx = zoom_profile_focal_for_ratio(zoomRatio);
      ...
      os << "{\"ok\":true"
         ...
         << ",\"zoom_confidence\":" << zoomConf
         << ",\"zoom_source\":\"" << json_escape(zoomSource) << "\""
         << ",\"focal_px\":" << focalPx
         << ",\"focal_min_px\":" << prof.focal_min_px
         << ",\"focal_max_px\":" << prof.focal_max_px
         << ",\"zoom_sample_idx\":" << sampleIdx
         << ",\"zoom_sample_count\":" << sampleCount
         << ",\"zoom_steps_since_home\":" << g_zoomStepsSinceHome.load()
         << ",\"zoom_move_busy\":" << (g_zoomMoveBusy.load() ? "true" : "false")
         << ",\"zoom_last_home_ms\":" << g_zoomLastHomeMs.load()
         << ",\"zoom_last_move_ms\":" << g_zoomLastMoveMs.load()
         << "}";
      const std::string body = os.str();
      const std::string hdr = "HTTP/1.1 200 OK\r\nContent-Type: application/json\r\nCache-Control: no-store\r\nConnection: close\r\nContent-Length: " + std::to_string(body.size()) + "\r\n\r\n";
      send_all(cfd, hdr.data(), hdr.size()); send_all(cfd, body.data(), body.size()); ::close(cfd); return;
    } else if (method == "POST") {
      double zr = g_zoomRatio.load(), zc = g_zoomConfidence.load();
      extract_json_double_field(bodyReq, "zoom_ratio", &zr);
      extract_json_double_field(bodyReq, "zoom_confidence", &zc);
      const std::string src = trim(extract_json_string_field(bodyReq, "zoom_source"));
  ...
  if (path == "/api/zoom/go_to_sample") {
    ...
    if (ok) {
      ...
      {
        std::lock_guard<std::mutex> lk(g_zoomMasterProfileMtx);
        if (mode == "edge_wide") g_zoomSource = "go_to_sample_edge_wide";
        else if (mode == "edge_tele") g_zoomSource = "go_to_sample_edge_tele";
        else if (mode == "absolute_wide") g_zoomSource = "go_to_sample_absolute_wide";
        else g_zoomSource = "go_to_sample_relative";
      }

      if (mode == "edge_wide" || mode == "edge_tele") {
        g_zoomStepsSinceHome.store(0);
        g_zoomLastHomeMs.store(now_ms());
      } else if (mode == "absolute_wide") {
    ...
    if (ok) {
      os << ",\"zoom_ratio\":" << point.zoom_ratio
         ...
         << ",\"steps_since_home\":" << g_zoomStepsSinceHome.load();
      if (mode == "edge_wide") os << ",\"edge\":\"wide\"";
      else if (mode == "edge_tele") os << ",\"edge\":\"tele\"";
    } else {
  ...
  if (path == "/api/zoom/master_profile" && method == "GET") {
    ...
    if (!prof.loaded) {
      ...
      for (size_t i = 0; i < prof.points.size(); ++i) {
        if (i) os << ",";
        const auto& p = prof.points[i];
        os << "{\"profile_idx\":" << p.profile_idx << ",\"step\":" << p.profile_idx << ",\"zoom_ratio\":" << p.zoom_ratio << ",\"focal_px\":" << p.focal_px << "}";
      }
      os << "]}";
    }
    const std::string body = os.str();
  ...
  if (path == "/api/zoom_calibration") {
    if (method == "POST") {
      ...
      if (!g_zoomCalibInProgress.load()) {
        ...
        if (!extract_json_int_field(bodyReq, "anchor_tag_id", &p.anchor_tag_id)) extract_json_int_field(bodyReq, "near_tag_id", &p.anchor_tag_id);
        if (!extract_json_double_field(bodyReq, "anchor_distance_mm", &p.anchor_distance_mm)) extract_json_double_field(bodyReq, "near_distance_mm", &p.anchor_distance_mm);
        ...
        { const std::string cd = extract_json_string_field(bodyReq, "calibration_direction"); if (!cd.empty()) p.calibration_direction = cd; }
  ...
  if (path == "/api/zoom_calibration/build_master_profile") {
    ZoomAprilTagCalibParams settings = g_zoomCalibSettings;
    load_zoom_calib_settings(settings, g_cmdMaxZoom);
    clamp_zoom_calib_params(settings, g_cmdMaxZoom);
    const int targetCount = std::max(2, std::min(64, settings.samples));
    ...
    struct Measurement { double focal_px = 0.0; SourceRef source; };
    std::vector<std::string> profilesUsed;
    std::vector<std::pair<std::string, std::string>> profilesSkipped;
    std::vector<std::vector<Measurement>> bySample(targetCount);
    int pointsTotalRaw = 0;
    for (const auto& file : profileFiles) {
      std::ifstream in(file); if (!in.good()) continue;
      std::ostringstream ss; ss << in.rdbuf(); const std::string payload = ss.str();
      const std::string status = extract_json_string_field(payload, "status");
      int validRowsClean = 0; extract_json_int_field(payload, "valid_rows_clean", &validRowsClean);
      if (status != "ok" || validRowsClean < 2) { profilesSkipped.push_back({file, "status_failed_or_too_few_rows"}); continue; }
      profilesUsed.push_back(file);
      double anchorDistanceMm = 0.0; extract_json_double_field(payload, "anchor_distance_mm", &anchorDistanceMm);
      ...
      for (const auto& obj : objects) {
        int sampleIdx = -1;
        if (!extract_json_int_field(obj, "profile_idx", &sampleIdx) && !extract_json_int_field(obj, "step", &sampleIdx) && !extract_json_int_field(obj, "sample_idx", &sampleIdx)) extract_json_int_field(obj, "idx", &sampleIdx);
        bool anchorFound = true; extract_json_bool_field(obj, "anchor_found", &anchorFound);
        double focalPx = 0.0; extract_json_double_field(obj, "focal_px", &focalPx);
        if (sampleIdx < 0 || sampleIdx >= targetCount || !anchorFound || focalPx <= 0.0) continue;
        int sourceIdx = -1; if (!extract_json_int_field(obj, "source_idx", &sourceIdx)) extract_json_int_field(obj, "idx", &sourceIdx);
        bySample[sampleIdx].push_back({focalPx, {file, sourceIdx, anchorDistanceMm}});
        ++pointsTotalRaw;
      }
    }
    std::vector<double> finalFocal(targetCount, 0.0);
    std::vector<std::vector<Measurement>> cleanBySample(targetCount);
    int pointsTotalClean = 0;
    for (int i = 0; i < targetCount; ++i) {
      auto vals = bySample[i]; if (vals.empty()) continue;
      std::sort(vals.begin(), vals.end(), [](const Measurement& a, const Measurement& b){ return a.focal_px < b.focal_px; });
      cleanBySample[i] = vals; pointsTotalClean += static_cast<int>(vals.size());
      const size_t n = vals.size(); finalFocal[i] = (n % 2) ? vals[n/2].focal_px : ((vals[n/2 - 1].focal_px + vals[n/2].focal_px) / 2.0);
    }
    int available = 0; for (double f : finalFocal) if (f > 0.0) ++available;
    std::string body;
    if (available < 2) {
      std::ostringstream os; os << "{\"ok\":false,\"error\":\"too_few_master_points\",\"points_total_raw\":" << pointsTotalRaw << ",\"points_total_clean\":" << pointsTotalClean << ",\"output_points\":" << targetCount << ",\"samples_requested\":" << targetCount << "}"; body = os.str();
    } else {
      for (int i = 0; i < targetCount; ++i) if (finalFocal[i] <= 0.0) {
        int left = i - 1; while (left >= 0 && finalFocal[left] <= 0.0) --left;
        int right = i + 1; while (right < targetCount && finalFocal[right] <= 0.0) ++right;
        if (left >= 0 && right < targetCount) { double t = double(i - left) / double(right - left); finalFocal[i] = finalFocal[left] + (finalFocal[right] - finalFocal[left]) * t; }
        else if (left >= 0) finalFocal[i] = finalFocal[left]; else if (right < targetCount) finalFocal[i] = finalFocal[right];
      }
      const auto mm = std::minmax_element(finalFocal.begin(), finalFocal.end()); const double minF = *mm.first, maxF = *mm.second;
      auto writePoints = [&](std::ostringstream& out){
        for (int i = 0; i < targetCount; ++i) {
          double zr = (maxF > minF) ? ((finalFocal[i] - minF) / (maxF - minF)) : 0.0; if (i) out << ",";
          out << "{\"profile_idx\":" << i << ",\"step\":" << i << ",\"zoom_ratio\":" << std::fixed << std::setprecision(3) << std::max(0.0, std::min(1.0, zr)) << ",\"focal_px\":" << finalFocal[i] << ",\"sources\":[";
          for (size_t j = 0; j < cleanBySample[i].size(); ++j) { if (j) out << ","; const auto& s = cleanBySample[i][j].source; out << "{\"file\":\"" << json_escape(s.file) << "\",\"source_idx\":" << s.source_idx << ",\"anchor_distance_mm\":" << s.anchor_distance_mm << "}"; }
          out << "]}";
        }
      };
      std::ostringstream master;
      master << "{\"method\":\"apriltag_zoom_master_profile\",\"status\":\"ok\",\"samples_requested\":" << targetCount << ",\"profiles_used\":[";
      ...
      master << "],\"points_total_raw\":" << pointsTotalRaw << ",\"points_total_clean\":" << pointsTotalClean << ",\"output_points\":" << targetCount << ",\"focal_min_px\":" << minF << ",\"focal_max_px\":" << maxF << ",\"profile_points\":["; writePoints(master); master << "],\"points\":["; writePoints(master); master << "]}";
      std::ofstream mout(g_zoomMasterProfileFile, std::ios::trunc); mout << master.str(); load_zoom_master_profile();
      std::ostringstream os; os << "{\"ok\":true,\"output\":\"" << json_escape(g_zoomMasterProfileFile) << "\",\"samples_requested\":" << targetCount << ",\"output_points\":" << targetCount << ",\"profiles_used\":[";
      for (size_t i = 0; i < profilesUsed.size(); ++i) { if (i) os << ","; os << "\"" << json_escape(profilesUsed[i]) << "\""; }
      os << "],\"profiles_skipped\":["; for (size_t i = 0; i < profilesSkipped.size(); ++i) { if (i) os << ","; os << "{\"file\":\"" << json_escape(profilesSkipped[i].first) << "\",\"reason\":\"" << profilesSkipped[i].second << "\"}"; }
      os << "],\"points_total_raw\":" << pointsTotalRaw << ",\"points_total_clean\":" << pointsTotalClean << "}"; body = os.str();
    }
    const std::string hdr = "HTTP/1.1 200 OK\r\nContent-Type: application/json\r\nCache-Control: no-store\r\nConnection: close\r\nContent-Length: " + std::to_string(body.size()) + "\r\n\r\n";
    send_all(cfd, hdr.data(), hdr.size()); send_all(cfd, body.data(), body.size()); ::close(cfd); return;
  }

  if (path == "/api/zoom_profile") {
    const std::ifstream in(g_zoomAprilTagProfileFile);
    std::string body = g_zoomAprilTagProfileJson;
    if (in.good()) {
...
int main(int argc, char** argv) {
  ...
  save_zoom_calibration_profile();
  if (load_zoom_master_profile()) {
    g_zoomRatio = 0.0;
    load_zoom_runtime_state();
    if (g_zoomConfidence.load() <= 0.0) g_zoomConfidence = 1.0;
    if (g_zoomSource.empty()) g_zoomSource = "startup_wide_home";
    if (g_zoomRatio.load() < 0.0 || g_zoomRatio.load() > 1.0) g_zoomRatio = 0.0;

    g_zoomConfidence = 1.0;
    std::lock_guard<std::mutex> lk(g_zoomMasterProfileMtx);
    g_zoomSource = "startup_wide_home";
  }
