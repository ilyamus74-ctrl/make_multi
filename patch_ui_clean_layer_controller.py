...
bak = p.with_suffix(p.suffix + f".bak_before_clean_layer_controller_{int(time.time())}")
...
def remove_script_containing_marker(text, marker):
    removed = 0
    ...
    while marker in text:
        ...
        text = text[:start] + f"\n<!-- removed script containing {marker} -->\n" + text[end:]
        removed += 1
...
for marker in REMOVE_MARKERS:
    s, n = remove_script_containing_marker(s, marker)
    removed_total[marker] = n
...
(function () {
  ...
  function sanitizePtzFraming(ptz) {
    const out = {};

    for (const [k, v] of Object.entries(ptz || {})) {
      if (FRAMING_KEYS.has(k)) {
        out[k] = v;
      }
    }
  ...
  function mergedObjectPresets(settings) {
    return Object.assign({}, BUILTIN_OBJECT_PRESETS, settings.objectPresetsCustom || {});
  }

  function activeObjectPreset(settings) {
    const name = activeObjectPresetName(settings);
    const presets = mergedObjectPresets(settings);
    const preset = Object.assign({}, BUILTIN_OBJECT_PRESETS[name] || {}, presets[name] || {});

    preset.ptz = sanitizePtzFraming(preset.ptz || {});
  ...
  function uiValueNumber(id, fallback) {
    const el = $(id);
  ...
  function uiValueString(id, fallback) {
    const el = $(id);
  ...
  function presetFromCurrentUi(settings, name) {
    const presets = mergedObjectPresets(settings);
    const old = Object.assign({}, BUILTIN_OBJECT_PRESETS[name] || {}, presets[name] || {});

    const maxDet = uiValueNumber('operatorDetectionLimit', Number(old.max_detections || settings.operatorDetectionLimit || 10));
    const every = uiValueNumber('operatorDetectEvery', Number(old.detect_every_n_frames || settings.operatorDetectEvery || 1));
    const area = uiValueString('operatorDetectionAreaMode', String(old.detection_mode || settings.operatorDetectionAreaMode || 'full_frame'));
    const model = uiValueString('operatorModelSelect', String(old.model || old.operatorModel || settings.operatorModel || ''));
    ...
    return Object.assign({}, old, {
      ...
      ptz: sanitizePtzFraming(old.ptz || {})
    });
  }

  async function applyDetectionPreset(preset) {
  ...
  async function applyPtzFramingPreset(preset) {
    const ptz = sanitizePtzFraming(preset.ptz || {});

    if (!Object.keys(ptz).length) {
      return { ok: true, skipped: true };
    }

    return await postJson(`${autopilotBaseUrl()}/api/autopilot/config`, ptz);
  }

  function ensureObjectPresetPanel() {
    if ($('objectPresetPanel')) return;

    const detMenu = $('detectionMenu') || document.querySelector('.controls-panel') || document.body;
  ...
  function ensureSearchPresetPanel() {
    if ($('searchPresetPanel')) return;

    const objectPanel = $('objectPresetPanel');

    if (!objectPanel || !objectPanel.parentNode) {
      return;
    }
  ...
  async function selectObjectPreset(name) {
    ...
    const backend = await applyDetectionPreset(preset);
    const ptz = await applyPtzFramingPreset(preset);
    ...
    syncDetectionUiFromPreset(preset);
    paintObjectPreset(name);
    await syncPtzRunState();
    ...
    return { ok: true, name, preset, backend, ptz };
  }

  function syncDetectionUiFromPreset(preset) {
  ...
  async function saveDetectionToPreset(reason = 'manual') {
    let settings = await readSettings();
    const name = activeObjectPresetName(settings);
    const preset = presetFromCurrentUi(settings, name);
    ...
    if (state) {
      state.textContent = ok ? `preset: ${name} saved OK` : `preset: ${name} SAVE FAIL`;
    }
    ...
    return { ok, name, preset, backend, ptz, saved };
  }
  ...
  async function setSearchPreset(name) {
    ...
    const st = $('searchPresetState');

    if (st) {
      st.textContent = `search: ${name}`;
    }

    const line = $('ptzStateLine');

    if (line) {
      line.textContent = `search preset saved: ${name}`;
    }
  }

  async function startPtzArmed() {
    const settings = await readSettings();
    ...
    if (line) {
      line.textContent = 'PTZ armed: daemon will start tracking/search';
    }
  }

  async function stopPtzArmed() {
    const settings = await readSettings();
    ...
    if (line) {
      line.textContent = 'PTZ stopped/disarmed';
    }
  }

  async function syncPtzRunState() {
  ...
  async function syncAllUiFromSettings() {
    const settings = await readSettings();
    const { name, preset } = activeObjectPreset(settings);
  ...
  function bindCleanHandlers() {
    ...
    document.addEventListener('click', async (e) => {
      ...
      const name = btn.dataset.cleanObjectPreset;

      try {
        await selectObjectPreset(name);
      } catch (err) {
        console.error('[clean-object-preset]', err);
    ...
    const saveBtn = $('objectPresetCleanSaveBtn');

    if (saveBtn && !saveBtn.__cleanBound) {
      saveBtn.__cleanBound = true;
    ...
    const searchSelect = $('searchPresetSelect');

    if (searchSelect && !searchSelect.__cleanBound) {
      searchSelect.__cleanBound = true;
    ...
    const logsBtn = $('objectTrackingLogsBtn');

    if (logsBtn && !logsBtn.__cleanBound) {
      logsBtn.__cleanBound = true;
    ...
    if (start && !start.__cleanBound) {
      start.__cleanBound = true;
      start.onclick = null;
    ...
    if (stop && !stop.__cleanBound) {
      stop.__cleanBound = true;
      stop.onclick = null;
    ...
    document.addEventListener('change', (e) => {
      ...
      if ([
        ...
        window.clearTimeout(window.__cleanPresetSaveTimer);

        window.__cleanPresetSaveTimer = window.setTimeout(() => {
          saveDetectionToPreset(id).catch((err) => {
            console.error('[clean-autosave-preset]', err);
          });
        }, 600);
      }
    }, true);
  ...
  function bootCleanLayer() {
    installStyle();
    ensureObjectPresetPanel();
    ensureSearchPresetPanel();
    bindCleanHandlers();

    syncAllUiFromSettings().catch((err) => console.error('[clean-sync]', err));
  }

  if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', bootCleanLayer, { once: true });
  } else {
    bootCleanLayer();
  }

  setTimeout(bootCleanLayer, 500);
  setTimeout(bootCleanLayer, 1500);
  setInterval(() => syncPtzRunState().catch(() => {}), 1500);
...
if "PTZ_CLEAN_LAYER_CONTROLLER_START" not in s:
    if "</body>" not in s:
...
for marker, count in removed_total.items():
    print(marker, "=", count)
