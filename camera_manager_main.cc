...
static nlohmann::json serializeGlobalObjects(const std::vector<GlobalObject>& objects) {
  ...
  for (const auto& obj : objects) {
    ...
    for (const auto& [cam_id, bbox] : obj.camera_detections) {
      cams.push_back({{"camera", cam_id}, {"bbox", {bbox.x, bbox.y, bbox.width, bbox.height}}});
    }
    item["cameras"] = cams;
...
g_server.Post("/api/record/start", [](const httplib::Request& req, httplib::Response& res){
    nlohmann::json resp;  // ← ИЗМЕНЕНО
    try {
        auto j = nlohmann::json::parse(req.body);  // ← ИЗМЕНЕНО
        int duration = j.value("duration", 30);
        // ОЧИЩАЕМ ПАПКУ REC ПЕРЕД ЗАПИСЬЮ
        std::error_code ec;
//        std::filesystem::path rec_dir = "./rec";
        std::filesystem::path rec_dir = "/tmp/rec";
        
        if (std::filesystem::exists(rec_dir)) {
            printf("Cleaning rec directory before recording...\n");
            int deleted_files = 0;
            ...
            printf("Deleted %d old AVI files\n", deleted_files);
        } else {
            std::filesystem::create_directories(rec_dir, ec);
            printf("Created rec directory\n");
        }
        
        // Запускаем запись на всех активных камерах
        auto cams = g_mgr.configuredCameras();
        int started = 0;
        
        for(auto& c : cams) {
            if(c.present && c.mode == CameraManager::CamConfig::Mode::Calibration) {
                std::string url = "http://localhost:" + std::to_string(c.det_port) + "/api/record/start";
                
                // Делаем HTTP запрос к камере
                httplib::Client client("localhost", c.det_port);
                client.set_connection_timeout(5, 0);
                client.set_read_timeout(5, 0);
                
                nlohmann::json cam_req;  // ← ИЗМЕНЕНО
                ...
                if(cam_res && cam_res->status == 200) {
                    started++;
                    printf("Started recording on camera %s (port %d)\n", c.id.c_str(), c.det_port);
                } else {
                    printf("Failed to start recording on camera %s (port %d)\n", c.id.c_str(), c.det_port);
                }
            }
        }
        
        resp["status"] = "ok";
        resp["started_cameras"] = started;
        resp["duration"] = duration;
        resp["message"] = "Recording started on " + std::to_string(started) + " cameras";
...
g_server.Post("/api/record/stop", [](const httplib::Request&, httplib::Response& res){
    nlohmann::json resp;  // ← ИЗМЕНЕНО
    ...
    for(auto& c : cams) {
        if(c.present && c.mode == CameraManager::CamConfig::Mode::Calibration) {
            ...
            auto cam_res = client.Post("/api/record/stop", "", "application/json");
            if(cam_res && cam_res->status == 200) {
                stopped++;
                printf("Stopped recording on camera %s (port %d)\n", c.id.c_str(), c.det_port);
            }
        }
    }
    
    resp["status"] = "ok";
    resp["stopped_cameras"] = stopped;
    resp["message"] = "Recording stopped on " + std::to_string(stopped) + " cameras";
    res.set_content(resp.dump(), "application/json");
});
...
g_server.Post("/api/calibration/start-auto", [](const httplib::Request&, httplib::Response& res){
    ...
    try {
        ...
        // Создаем CalibrationWatcher если его нет
        if (!g_calib_watcher) {
        ...
        if (g_calib_watcher->isProcessing()) {
            resp["status"] = "error";
            ...
            if (g_calib_watcher->startCalibration(params)) {
                resp["status"] = "ok";
