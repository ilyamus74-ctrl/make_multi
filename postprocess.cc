...
static bool readLine(FILE *fp, std::string &line)
{
    ...
    if (ch == EOF && (line.empty() || ferror(fp)))
    {
        line.clear();
        return false;
    }
...
static int readLines(const char *fileName, std::array<std::string, OBJ_CLASS_NUM> &lines)
{
    ...
    int i = 0;
    std::string line;
    while (i < OBJ_CLASS_NUM && readLine(file, line))
    {
        lines[i++] = line;
    }
    fclose(file);
...
static int loadLabelName(const char *locationFilename, std::array<std::string, OBJ_CLASS_NUM> &label)
{
    printf("load lable %s\n", locationFilename);
