...
static void autopilot_loop(){WsClient ws; auto last=std::chrono::steady_clock::now(); double ix=0,iy=0,pex=0,pey=0; int prevp=0,prevt=0; bool sent_stop=false; double smoothScale=1.0; while(g.run){auto start=std::chrono::steady_clock::now(); TrackerState ts; std::string body; bool ok=http_get(cfg.tracker_host,cfg.tracker_port,"/api/tracker/state",body)&&parse_tracker(body,ts); int pan=0,tilt=0,basePan=0,baseTilt=0; double ex=0,ey=0; ZoomState zs; fetch_zoom_state(zs); const double rawScale=compute_zoom_speed_scale(zs); const double speedScale=(cfg.zoom_scale_smoothing<=0.0)?rawScale:(smoothScale=smoothScale*(1.0-cfg.zoom_scale_smoothing)+rawScale*cfg.zoom_scale_smoothing); bool active=g.enabled.load(); bool needSend=false; long long manualExpire=0; bool manualOn=false; {std::lock_guard<std::mutex> lk(g.m); manualOn=g.manual_active; manualExpire=g.manual_expires_ms;} if(manualOn && now_ms()>=manualExpire){ send_bridge_j(0,0); std::lock_guard<std::mutex> lk(g.m); g.manual_active=false; g.manual_cmd_pan=0; g.manual_cmd_tilt=0; g.manual_expires_ms=0; } int effectiveMaxPan=std::max(1,(int)std::lround(cfg.max_pan*speedScale)); int effectiveMaxTilt=std::max(1,(int)std::lround(cfg.max_tilt*speedScale)); int effectiveMaxAccel=std::max(1,(int)std::lround(cfg.max_accel*speedScale)); const int effectiveMinPan=cfg.min_pan>0?std::max(1,(int)std::lround(cfg.min_pan*speedScale)):0; const int effectiveMinTilt=cfg.min_tilt>0?std::max(1,(int)std::lround(cfg.min_tilt*speedScale)):0; if(active && ok && ts.mode=="TRACKING" && ts.valid){double cx=(ts.l+ts.r)*0.5, cy=(ts.t+ts.b)*0.5; const double nx=cx/std::max(1,cfg.width); const double ny=cy/std::max(1,cfg.height); ex=(nx-cfg.target_x)*2.0; ey=(ny-cfg.target_y)*2.0; if(std::abs(ex)<cfg.deadzone) ex=0; if(std::abs(ey)<cfg.deadzone) ey=0; auto now=std::chrono::steady_clock::now(); double dt=std::chrono::duration<double>(now-last).count(); last=now; dt=std::max(0.02,std::min(0.2,dt)); ix+=ex*dt; iy+=ey*dt; double dx=(ex-pex)/dt, dy=(ey-pey)/dt; pex=ex; pey=ey; basePan=int(std::lround(cfg.kp*ex+cfg.ki*ix+cfg.kd*dx)); baseTilt=int(std::lround(cfg.kp*ey+cfg.ki*iy+cfg.kd*dy)); pan=std::max(-effectiveMaxPan,std::min(effectiveMaxPan,basePan)); tilt=std::max(-effectiveMaxTilt,std::min(effectiveMaxTilt,baseTilt)); if(ex!=0.0 && effectiveMinPan>0 && std::abs(pan)<effectiveMinPan) pan=(ex>0)?effectiveMinPan:-effectiveMinPan; if(ey!=0.0 && effectiveMinTilt>0 && std::abs(tilt)<effectiveMinTilt) tilt=(ey>0)?effectiveMinTilt:-effectiveMinTilt; if(cfg.invert_pan) pan=-pan; if(cfg.invert_tilt) tilt=-tilt; pan=std::max(prevp-effectiveMaxAccel,std::min(prevp+effectiveMaxAccel,pan)); tilt=std::max(prevt-effectiveMaxAccel,std::min(prevt+effectiveMaxAccel,tilt)); prevp=pan; prevt=tilt; sent_stop=false; needSend=true;} else {ix=iy=pex=pey=0; prevp=prevt=0; if(!sent_stop){pan=0;tilt=0; sent_stop=true; needSend=true;} else {std::this_thread::sleep_for(std::chrono::milliseconds(10));}}
